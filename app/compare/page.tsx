...
export default function ComparePage() {
  ...
  const addAbility = (abilityId: string) => {
    if (!selectedAbilities.includes(abilityId) && selectedAbilities.length < 4) {
  ...
  const selectedAbilityData = selectedAbilities.map((id) => ccAbilities.find((a) => a.id === id)).filter(Boolean)

  const availableAbilities = ccAbilities.filter((a) => !selectedAbilities.includes(a.id))

  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-2 text-balance">Compare CC Abilities</h1>
      ...
      <Card className="p-6 mb-6">
        <div className="flex items-center gap-4">
          <Select value="" onValueChange={(value) => addAbility(value)} disabled={selectedAbilities.length >= 4}>
            ...
            <SelectContent>
              {availableAbilities.map((ability) => (
                <SelectItem key={ability.id} value={ability.id}>
                  <span className={classColors[ability.class]}>{ability.class}</span> - {ability.ability}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          {selectedAbilities.length > 0 && (
            <Button variant="outline" onClick={() => setSelectedAbilities([])} className="gap-2">
              <X className="h-4 w-4" />
              Clear All
            </Button>
          )}
        ...
        {selectedAbilities.length >= 4 && (
          <p className="text-sm text-muted-foreground mt-2">Maximum of 4 abilities can be compared at once</p>
        )}
      </Card>
      ...
      {selectedAbilityData.length === 0 ? (
        <Card className="p-12">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-lg">No abilities selected</p>
            <p className="text-sm mt-2">Select abilities from the dropdown above to start comparing</p>
          </div>
        </Card>
      ) : (
            ...
            <Card key={ability.id} className="relative">
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <p className={`text-lg font-semibold ${classColors[ability.class]}`}>{ability.class}</p>
                </div>
                ...
                <div className="pt-4 border-t border-border space-y-3">
                  <div>
                    <p className="text-sm text-muted-foreground mb-1">DR Category</p>
